...
let sight_lines g x y =
  let h = Grid.height g in
  let w = Grid.width g in
  let get_on_col x y = Grid.get_exn g ~col:x ~row:y in
  let get_on_row y x = Grid.get_exn g ~col:x ~row:y in
  [
    (* left *)
...
let is_visible_global g x y =
  let v = Grid.get_exn g ~col:x ~row:y in
  let is_vis_slice = List.for_all ~f:(fun x -> x < v) in
  sight_lines g x y |> List.map ~f:is_vis_slice |> or_bools

let count_visible g =
  Grid.positions g |> List.count ~f:(fun (x, y) -> is_visible_global g x y)
